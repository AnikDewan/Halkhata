...
export default function CustomersScreen() {
  ...
  const customersQuery = db.select({
    ...
    totalGiven: sql<number>`COALESCE(SUM(CASE WHEN ${transactions.type} = 'given' THEN ${transactions.amount} ELSE 0 END), 0)`.mapWith(Number),
    totalReceived: sql<number>`COALESCE(SUM(CASE WHEN ${transactions.type} = 'received' THEN ${transactions.amount} ELSE 0 END), 0)`.mapWith(Number),
  })
  .from(customers)
      ...
      <View className="flex-1 px-4 pt-4">
        ...
        {filteredCustomers.length === 0 ? (
          ...
          >
            ...
            <Text className="text-foreground font-bold text-base">No customers found</Text>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <View className={cn(
                        "w-1.5 h-16",
                        balance === 0 ? "bg-transparent" : balance > 0 ? "bg-danger" : "bg-success"
                      )} />

                      <View className="h-10 w-10 rounded-full bg-primary items-center justify-center ml-3 mr-3 shadow-xs">
                        <Text className="text-white font-bold text-xs">{getInitials(item.name)}</Text>
                      </View>

                      <View className="flex-1 py-3">
                        <Text className="text-foreground font-bold text-sm">{item.name}</Text>
                        {item.phone && (
                          <Text className="text-muted text-xs mt-0.5 font-medium">{item.phone}</Text>
                        )}
                      </View>

                      <View className="items-end py-3 px-4">
                        {balance === 0 ? (
                          <Text className="text-muted text-xs font-bold">Settled</Text>
                        ) : (
                          <>
                            <Text
                              className={cn(
                            ...
                            >
                              {balance > 0 ? 'To Receive' : 'To Give'} {formatMoney(Math.abs(balance)).replace(/[^\d.,]/g, '')}
                            </Text>
