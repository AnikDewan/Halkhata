  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <View className="flex-1 px-4 pt-4">
        ...
        <View className="flex-row bg-card border border-border p-1 rounded-2xl mb-3 shadow-xs" style={{ borderCurve: 'continuous' }}>
            ...
            >
              <Text
                className={cn(
                  ...
                  typeFilter === type ? "text-white" : "text-foreground-secondary"
                )}
              >
                {type === 'all' ? 'All' : type === 'given' ? 'Given' : 'Received'}
              </Text>
            </Pressable>
          ))}
        </View>
            ...
            return (
              ...
              >
                <Text
                  className={cn(
                    "text-xxs font-bold",
                    timeFilter === time ? "text-white" : "text-foreground-secondary"
                  )}
                >
                  {labels[time]}
                </Text>
        ...
        {filteredTransactions.length === 0 ? (
          ...
          >
            ...
            <Text className="text-foreground font-bold text-base">No entries found</Text>
          ...
          <View className="flex-1 mb-2" style={{ minHeight: 200 }}>
            <FlashList
              ...
              renderItem={({ item, index }) => (
                ...
                >
                  <Pressable
                    onPress={() => router.push(`/customer/${item.customerId}` as any)}
                  ...
                  >
                    ...
                    <View className="flex-1 py-3 px-3">
                      <View className="flex-row items-center gap-1.5 flex-wrap">
                        <Text className="text-foreground font-bold text-sm">{item.customerName}</Text>
                      </View>
                      <Text className="text-muted text-xs mt-1" numberOfLines={1}>
                        {item.description || (item.type === 'given' ? 'Given' : 'Received')}
                      </Text>
                    </View>
                    <View className="items-end py-3 px-4">
                      <View className="flex-row items-center gap-1">
                        {item.type === 'given' ? (
                          ...
                          <ArrowDown size={11} color={c.success} strokeWidth={3} />
                        )}
                        <Text
                          className={cn(
                            "font-extrabold text-sm tabular-nums",
                            item.type === 'given' ? "text-danger" : "text-success"
                          )}
                        >
                          {formatMoney(item.amount).replace(/[^\d.,]/g, '')}
                        </Text>
                      </View>
                      <Text className="text-muted text-xxs mt-1 font-semibold">{formatDate(item.createdAt)}</Text>
                    </View>
                  </Pressable>
                </Animated.View>
              )}
